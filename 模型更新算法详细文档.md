...
> ⚠️ **本文档已过期（历史存档）**：其描述的 `update_elements_to_database` /
> `process_model_updates` 分类刷新流程已从代码中移除。当前增量更新的权威实现见：
> `src/data_interface/increment_pipeline.rs`（落库 + 水位）、
> `src/data_interface/model_refresh.rs`（`conservative_regen → owner_regen` 刷新）、
> `src/data_interface/model_impact.rs`（"宁多勿漏"影响判定）、
> `src/data_interface/manual_update.rs`（手动预览/执行），以及
> `docs/adr/ADR-001-dbnum-update-state.md`。下文仅作历史参考。
