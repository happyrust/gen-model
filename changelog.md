...
- 数据批次支持受控并发调度和 panic 隔离，缩短多库增量处理时间且避免单批失败阻塞队列。
- 新增 `DELETE /api/v1/dbnums/{dbnum}/data` 快速清库接口，按 ref0 范围分阶段删除。
- 新增 E3D 增量夹具套件、节点生成/房间归属探针和手动服务启动脚本。
- 模型查询 HTTP 接口补齐实例、关系和生成根信息，便于现场诊断。

### 修复

- 修复 staged 增量中房间、实例、布尔、管件关系和 AABB 写回的一致性问题。
- SurrealDB/SUL_DB 读写增加瞬时传输错误重试，降低连接抖动导致的批次失败。
- 修复按需解析的记录边界、owner 元数据保留、目录缺失记录及模型数据库重复初始化问题。

### 发布

- Windows CI 直接使用预编译 OpenCASCADE 与 protoc；标签发布复用主分支构建产物，不再重复编译。
