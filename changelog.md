...
- 修复 CATA 页面批量预取误用局部矩阵世界变换，导致 8000 的 `/1RX03-LCT` 1907 条
  模型关系全部落在原点的问题；批量 API 现复用持久化/暂存感知的权威解析器。新的空
  RocksDB 完整初始化后 1907/1907 条位移非零，Plant UI 模型恢复空间分布。同步补齐
...
- 修复空 RocksDB 初始化 8000 时 CATA 准备退化为“766 个生成根逐根重读 DESI/CATA”及
  ...
  按 dbnum 每 1000 行分页读取已落库属性，合并为 160 个 CATA 种子后只运行一次页式闭包；
  不套用稳态增量暂存入口，也不回读 DESI 文件。

- 修复目录 Manifold 布尔在失败策略门之前单独读取正实体 `.mesh`：文件缺失过去以裸
  `os error 2` 上浮，导致一个 BRAN 拖垮 16 根批次并重复约 27 秒的 CATA 生成。现在
  精确属性顶点判定与正实体载入合并为一次读取，BestEffort 初始化将缺失网格记入
  `geom_error(bool_pos)`、标记 `bad_bool` 并继续；Required 暂存窗口仍上浮阻断。

- 优化一次性初始化的坏模型收口：生成根失败经 16 根有界页隔离后写入
  `geom_error(kind=generation_root)` 并收口该工作单，不再在同一次启动内原地重试五轮、
  每 60 秒阻塞等待 `model_ready`；后续重新生成成功会自动销账。现场否决了 64 根大页：
  含坏根时首次失败约 60 秒，明显劣于 16 根页的故障隔离边界。

- 修复 dbnum 初始化完成后，模型批次仍按生成根重开 DESI/CATA 源文件、重复计算依赖
  闭包并覆盖 SurrealDB 的性能回退。普通初始化统一为“完整解析落 RocksDB/SurrealDB →
  直接生成模型”；只有未提交的暂存增量窗口准备原子提交依赖，模型缺失按 `geom_error`
  记录，不再从持久模型路径回退分页文件解析。

- 修正 Dabacon 文件头 `0x34` 的单位语义：该字段是 32 位 word 数，`512`
  对应物理页长 `2048` 字节。页引擎、快照、hint 和日志采用带单位的变量名，
  并保留完整 session 边界校验，避免把 `page_no * 512` 偶遇的页型魔数当成真页。

- 修复单位几何三角化/网格持久化失败只设置 `inst_geo.bad`、未进入 `geom_error` 的
  静默缺口；现在按 geo hash 记录错误、受影响模型列表和累计次数，成功重生成后销账。

- CATA 定位与按需预加载统一改用页式快照：去掉打开时对旧会话页解析器的二次校验，
  以页式引擎已验证的 session/index root 为唯一读取面，并在打开前后校验文件身份。
  AMS 8000 实库已连续消化模型工作单，CATA 闭包实际解析 950/1908 个元素且未再出现
  `SessionPageData` 溢出 panic。主仓、Python、解析包和 IO 包的本地联调依赖改为 path，
  不再为此联调链升级 git rev。
- 增加 OCC 退役 caliber 原子重建只读预检：统一核对六类复用曲面的 caliber、旧引用、
  坏身份、未收口队列、孤儿和 mesh 文件；维护窗口用 `-RequireReady` 失败闭合。双库预检
  分别检出 29/392 个旧身份，避免把局部重生成误报为整库迁移完成。
- RVM live 门可显式选择数据库 endpoint 与独立 mesh 目录，并新增从生成根重建 CWALL 的
  隔离副本入口。7997 副本的 20/20 GWALL union 已在无 OCC 口径通过，且三件大体量
  布尔网格与 8009 产物 SHA-256 完全一致；WALL/STWALL 可在没有历史派生关系时从各
  副本自己的 PE/CATA/SPINE 源属性走生产解析器对拍，两副本现已关闭 T046–T048。
- GWALL 自交倒角按 NonZero 填充拆成简单轮廓，并把分区边映射回 libgm 光顺组后做
  属性保真 Manifold union，不再生成开口网格或重叠分量的内部假表面。同步恢复普通
  布尔网格的 0.1/0.01mm 两档坐标口径，展开属性网格保持 f64 精度；定向重生成现在
  能复活并清除 `bad_bool`；现场另两堵历史坏墙及较大库副本的一块 FLOOR 也已重算成功。AMS 1112 三堵大体量
  GWALL 的无 OCC gen→RVM p95 分别恢复到 0.1/9.3/167.5mm，全部通过既有门限。
