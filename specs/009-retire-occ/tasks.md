...
- [ ] T053（新，2026-08-24 从 T038a 的验收缺口里撞出来；**T041 / G3 的范围要重写**）
  ...
  **2026-08-25：范围已数完，两个量级的说法可以收掉——是同一个量级。**
  证据 `docs/evidence/2026-08-25-t053-segment-identity-scope.md`（库 A 副本 @8039，
  `FACET_TOL_MM = 0.5`，规则逐行照抄 `libgm_discretise` 并先跑其单测对照表自检）。

  | 变体 | 行 | 实例 | 段数等价类 | 段数区间 |
  |---|---:|---:|---:|---|
  | 单位柱 `inst_geo:⟨2⟩`（`PrimLCylinder`+`PrimSCylinder` 双键同一行） | 1 | 20,661 | **44** | 8…456 |
  | `PrimCTorus` | 95 | 664 | **102** | 环向 2…82 |
  | `PrimRTorus` | 167 | 1,945 | **174** | 3…256 |
  | `PrimLSnout` | 112 | 1,201 | **133** | 8…160 |
  | `PrimDish` | 17 | 102 | **21** | 绕轴 8…492 |
  | 合计 | **392** | | **474** | |

  **改键后 `.mesh` 392 → 474（+82）**，其中 43 来自柱那一行。炸开的是少数行
  （环面 6/95、矩形环面 5/167、Snout 18/112、碟 1/17），其余仍是 1 → 1。
  与已有的 3,896 行挤出比仍可忽略——ADR-044 决策 2 成立，D1「不设独立窗口、
  同批重建」不必回开。

  **为什么不是 T045 的 37：那个数只算了正体。** `insts_flat` 的回填式带
  `geo_type = 'Pos'`（`pdms_inst::sweep_inst_relate_flat`），是**读侧投影**；
  而 `.mesh` 按 `geo_hash` 存，负体也吃——`apply_cata_neg_boolean_manifold` 取操作数
  走 `out->geo_relate where geo_type == "Neg" or geo_type == "CataCrossNeg"` 再
  `record::id(out)`，与正体同一张表。按 `geo_type` 拆开：**只算 Pos/Compound 是
  392→460（柱 1→37，与 T045 逐位吻合）；只算负体族 392→432；全部吃 `.mesh` 的
  392→474（柱 1→44）**。排期按最后一档。差异集中在负体柱：`24381_40090` 等
  十二件是 8,000–41,800 mm 的圆柱形挖空，`insts_flat` 上一件都不该有。
  负体分错段数正是 ADR-044 要治的「共面留一层壁」，不能不算。

  **顺带把 `insts_flat` 的 11,992 个空数组判了：不是缺陷。** 逐行对着回填式的四道
  过滤分类——11,979 行（99.9%）的边**全是负体**，13 行有 Pos 边但全部不可见，
  「有合格边却仍为空」与「`booled_id` 有效却仍为空」两个缺陷桶**都是 0**。
  空数组是这些行的正确终态。
  真残留在另一头：`insts_flat = NONE` 的 1,479 行里 **40 行对读者可见**
  （`aabb.d != none`），正是清扫段 `WHERE` 圈的那一类，也正是 `pdms_inst` 那条
  live 断言禁止残留的那一类；其中三行还带 `booled_id`。静态副本判不出是缺陷还是
  清扫没跑，**只报不判**，归 ADR-041 / ADR-043 + `specs/025-insts-flat-invalidation/`。
