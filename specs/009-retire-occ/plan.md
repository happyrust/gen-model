...
> **2026-08-25 修订（T053 范围盘点）**：上表两处要改口径，证据
> `docs/evidence/2026-08-25-t053-segment-identity-scope.md`。
>
> 1. **G3 不只是柱。** 参与单位复用的曲面原语共五类，库 A 逐类数完是
>    柱 1→**44**、`PrimCTorus` 95→**102**、`PrimRTorus` 167→**174**、
>    `PrimLSnout` 112→**133**、`PrimDish` 17→**21**，**合计 392 → 474（+82）**。
>    量级仍然可忽略（对比 3,896 行挤出），裁决不变，但排期按 82 算而不是 36。
> 2. **库 A 那一列的 37 只算了正体。** 它算在 `inst_relate.insts_flat[]` 上，
>    而那个投影带 `geo_type = 'Pos'`（`pdms_inst::sweep_inst_relate_flat`）。
>    `.mesh` 按 `geo_hash` 存，**负体也吃**——`apply_cata_neg_boolean_manifold`
>    取操作数走 `out->geo_relate where geo_type == "Neg" or "CataCrossNeg"` 再
>    `record::id(out)`，同一张表。改按 `geo_relate` 全量算：柱 427 个不同直径、
>    **44** 个等价类，直径上限 41,800 mm（`24381_40090` 那件是**负体**，
>    一个 41.8 m 的圆柱形挖空，所以 `insts_flat` 上确实不该有它）。
>    拆开是 Pos/Compound 392→460（柱 1→37，与 T045 逐位吻合）、
>    负体族 392→432、全部 392→474。**排期按全部记。**
