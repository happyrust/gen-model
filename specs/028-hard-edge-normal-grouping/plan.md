...
- 交线上**新生成**的顶点：按上面的表示法，交线两侧本来就来自不同操作数的属性行，
  所以「交线是硬边」应当是**自然落出来的**，不需要额外规则。这条待探针 3 证实。
  若证伪，就要显式把交线边标硬，而那正是 libgm `normaliseStage2` /
  `isTangentDiscontinuity`（固定 22.5°）干的活，届时那条常量有了自己的落点。
