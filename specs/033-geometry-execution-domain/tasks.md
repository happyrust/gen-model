...
- [x] T002 [P] `src/fast_model/concurrency.rs`：新增许可**持有**时长累计
      ...
      2026-08-27 补：持有改为**读时结算**。原先只在许可 `Drop` 时累加，快照落在长
      布尔段中间时正被占着的那几张读数是 0；T004 的取样恰恰在 CPU 密集段内，利用率
      会被系统性低估，0.7 验收线可能把达标的配置判成不达标。账本改记「已归还许可的
      持有之和」与「在飞许可取得时刻之和」（一把 std Mutex，两者耦合、分开读会撕），
      读数时补上「在飞张数 × 当下 − 取得时刻之和」。两条回归测试各钉一个形态：段中
      读数不为 0、一张许可占满整窗读作 1.0；退回旧写法双双转红（实测 0us / Some(0.0)）。
      `--lib fast_model::concurrency` 十一条全绿。
