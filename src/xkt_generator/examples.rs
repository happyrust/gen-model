...
pub async fn create_table_example() -> anyhow::Result<()> {
    ...
    // 创建实体
    let mut table_entity = XKTEntity::new(
        "table_001".to_string(),
        "桌子-001".to_string(),
        "FURNITURE".to_string(),
    );
    ...
    xkt_file
        .save_to_file("examples_output/table_example.xkt", true)
        .await?;
...
pub async fn create_piping_system_example() -> anyhow::Result<()> {
    ...
    let cylinder_geometry =
        XKTGeometry::create_cylinder("cylinder_geometry".to_string(), 1.0, 1.0, 16);
    ...
    // 创建主管道
    let mut main_pipe_mesh = XKTMesh::new(
        "main_pipe_mesh".to_string(),
    ...
    // 创建分支管道
    let mut branch_pipe_mesh = XKTMesh::new(
        "branch_pipe_mesh".to_string(),
        "cylinder_geometry".to_string(),
    );
    ...
    // 创建实体
    let mut main_pipe_entity = XKTEntity::new(
        "pipe_main_001".to_string(),
        "主管道-001".to_string(),
        "PIPE".to_string(),
    );
    ...
    let mut branch_pipe_entity = XKTEntity::new(
        "pipe_branch_001".to_string(),
        "分支管道-001".to_string(),
        "PIPE".to_string(),
    );
    ...
    let mut pump_entity = XKTEntity::new(
        "pump_001".to_string(),
        "离心泵-001".to_string(),
        "EQUIPMENT".to_string(),
    );
    ...
    std::fs::create_dir_all("examples_output").ok();
    xkt_file
        .save_to_file("examples_output/piping_system_example.xkt", true)
        .await?;
...
pub async fn create_factory_layout_example() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all("examples_output").ok();
    xkt_file
        .save_to_file("examples_output/factory_layout_example.xkt", true)
        .await?;
...
pub async fn example_generate_xtk_from_database() -> anyhow::Result<()> {
    use crate::fast_model::gen_model::{generate_xtk_by_dbno, generate_xtk_from_database};
...
pub async fn example_filtered_xtk_generation() -> anyhow::Result<()> {
    ...
    use aios_core::options::DbOption;
    use aios_core::{RefnoEnum, query_type_refnos_by_dbnum};
