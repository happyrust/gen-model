...
impl XKTWriter {
    ...
    /// 将 XKT 文件写入文件
    pub async fn write_to_file(&self, xkt_file: &XKTFile, path: &str, compress: bool) -> Result<()> {
    ...
    /// 写入二进制几何体数据
    fn write_binary_geometry_data(&self, buffer: &mut Vec<u8>, geometry: &super::XKTGeometry) -> Result<()> {
        // 写入几何体ID长度和ID
...
impl XKTReader {
    ...
    pub fn validate_header(data: &[u8]) -> Result<bool> {
        ...
        if version != XKT_VERSION {
            println!("Warning: XKT version mismatch. Expected {}, got {}", XKT_VERSION, version);
        }
