...
async fn test_basic_xkt_generation_impl() -> anyhow::Result<()> {
    ...
    let cylinder_geometry =
        XKTGeometry::create_cylinder("cylinder_geometry".to_string(), 0.3, 2.0, 12);
    ...
    let mut equipment_mesh = XKTMesh::new(
        "equipment_mesh".to_string(),
        "cylinder_geometry".to_string(),
    );
    ...
    // 创建实体
    let mut pipe_entity = XKTEntity::new(
        "pipe_001".to_string(),
        "管道-001".to_string(),
        "PIPE".to_string(),
    );
    ...
    let mut valve_entity = XKTEntity::new(
        "valve_001".to_string(),
        "阀门-001".to_string(),
        "VALVE".to_string(),
    );
    ...
    let mut equipment_entity = XKTEntity::new(
...
async fn test_complex_scene_generation_impl() -> anyhow::Result<()> {
    ...
    let geometries = vec![
        (
            "box_small",
            XKTGeometry::create_box("box_small".to_string(), 0.5, 0.5, 0.5),
        ),
        (
            "box_large",
            XKTGeometry::create_box("box_large".to_string(), 2.0, 1.0, 1.0),
        ),
        (
            "sphere_small",
            XKTGeometry::create_sphere("sphere_small".to_string(), 0.3, 12, 12),
        ),
        (
            "sphere_large",
            XKTGeometry::create_sphere("sphere_large".to_string(), 0.8, 20, 20),
        ),
        (
            "cylinder_thin",
            XKTGeometry::create_cylinder("cylinder_thin".to_string(), 0.1, 3.0, 8),
        ),
        (
            "cylinder_thick",
            XKTGeometry::create_cylinder("cylinder_thick".to_string(), 0.5, 1.5, 16),
        ),
    ];
    ...
    for (id, name, entity_type, geometry_id, position) in components {
        // 创建网格
        let mut mesh = XKTMesh::new(format!("{}_mesh", id), geometry_id.to_string());
...
async fn test_xtk_database_generation() -> anyhow::Result<()> {
    use crate::fast_model::gen_model::{ElementInfo, generate_xtk_from_database};
    ...
    // 创建测试用的参考号
    let test_refnos = vec!["12345/67890".into(), "23456/78901".into()];
    ...
    match create_geometry_from_geo_param("test_box", &[geo_instance]).await {
        Ok(geometry) => {
            ...
            assert_eq!(
                ...
                crate::xkt_generator::XKTGeometryType::Triangles
            );
