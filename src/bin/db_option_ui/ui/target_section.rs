        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                mode_changed |= ui
                    ...
                    .changed();
                mode_changed |= ui
                    .radio_value(&mut app.parse_mode, ParseMode::ManualDbNums, "指定数据库编号")
                    .changed();
                mode_changed |= ui
...
fn handle_mode_change(app: &mut ConfigApp) {
    match app.parse_mode {
        ParseMode::Auto => {
            ...
            app.option.debug_root_refnos = None;
        }
        ParseMode::ManualDbNums => {
...
fn render_debug_refnos(app: &mut ConfigApp, ui: &mut egui::Ui) {
    ui.label("debug_root_refnos (逗号或换行分隔)");
...
fn parse_debug_refnos(app: &mut ConfigApp) -> anyhow::Result<()> {
    ...
    if parsed.is_empty() {
        ...
        app.option.debug_root_refnos = Some(parsed);
    }
