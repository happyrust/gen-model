            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                if is_running {
                    ...
                    ui.add_enabled_ui(can_run, |ui| {
                        let button = egui::Button::new("▶ 开始运行")
...
fn simulate_task_progress(app: &mut ConfigApp) {
    ...
    unsafe {
        ...
        if !INITIALIZED && app.task_progress.status == TaskStatus::Running {
            app.task_progress.add_log(LogLevel::Info, "任务开始执行".to_string());
            app.task_progress.add_log(LogLevel::Info, format!("站点名称: {}", app.site_name));
            INITIALIZED = true;
        }
        ...
        if should_update && app.task_progress.status == TaskStatus::Running {
            if CURRENT_STEP < steps.len() {
                ...
                if CURRENT_STEP >= steps.len() {
                    ...
                    app.task_progress.add_log(LogLevel::Success, "✓ 任务执行成功完成".to_string());
                    CURRENT_STEP = 0;
                    LAST_UPDATE = None;
                    INITIALIZED = false;
                }
