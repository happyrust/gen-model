...
use crate::db;
use crate::models::{FieldState, ParseMode, StatusKind, StatusMessage, TaskProgress, LogLevel};
use crate::ui::{render_model_section, render_parse_section, render_site_selector, render_target_section, render_top_bar, render_task_control};
...
impl ConfigApp {
    ...
    pub fn complete_task(&mut self, success: bool) {
        use crate::models::{TaskStatus, LogLevel};
        if success {
            ...
            self.task_progress.add_log(LogLevel::Error, "任务被用户中止".to_string());
        }
    }
    ...
    fn update_option_from_mode(&mut self) -> Result<()> {
        match self.parse_mode {
            ParseMode::ManualDbNums => {
                let parsed = parser::parse_u32_list(&self.manual_db_nums.text)
                    .map_err(|err| {
                        self.manual_db_nums.set_error(err.to_string());
                        err
                    })?;
                self.option.manual_db_nums = if parsed.is_empty() { None } else { Some(parsed) };
                self.option.debug_root_refnos = None;
            }
            ParseMode::DebugRefnos => {
                let parsed = parser::parse_string_list(&self.debug_refnos.text);
                self.option.debug_root_refnos = if parsed.is_empty() { None } else { Some(parsed) };
        ...
        self.option.included_db_files = if parsed.is_empty() { None } else { Some(parsed) };
    ...
    fn update_document(&self, document: &mut DocumentMut) {
        ...
        serializer::set_bool(document, "apply_boolean_operation", self.option.apply_boolean_operation);
        ...
        serializer::set_bool(document, "save_spatial_tree_to_db", self.option.save_spatial_tree_to_db);
        ...
        serializer::set_usize(document, "gen_model_batch_size", self.option.gen_model_batch_size);
        serializer::set_string_list_value(document, "included_db_files", &self.option.included_db_files);
        serializer::set_u32_list_option(document, "manual_db_nums", &self.option.manual_db_nums);
        serializer::set_string_list_option(document, "debug_root_refnos", &self.option.debug_root_refnos);
    }
...
fn create_debug_refnos_field(option: &DbOption) -> FieldState<Vec<String>> {
    let text = option.debug_root_refnos.clone().unwrap_or_default().join(",\n");
    FieldState::new(text, option.debug_root_refnos.clone())
}

fn create_included_db_files_field(option: &DbOption) -> FieldState<Vec<String>> {
    let text = option.included_db_files.clone().unwrap_or_default().join(", ");
