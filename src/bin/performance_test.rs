...
async fn main() -> anyhow::Result<()> {
    ...
    // 获取参数
    let start_dbno: u32 = matches.get_one::<String>("start_dbno")
        .unwrap()
        .parse()
        .expect("起始数据库号必须是有效的数字");
    
    let end_dbno: u32 = matches.get_one::<String>("end_dbno")
        .unwrap()
        .parse()
...
/// 保存性能报告到文件
fn save_performance_report(stats: &[aios_database::test::PerformanceStats], filename: &str) -> anyhow::Result<()> {
    use std::fs::File;
    use std::io::Write;
