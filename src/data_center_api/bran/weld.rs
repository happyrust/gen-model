...
pub async fn get_data_center_weld_attr(refno: PdmsElement, bran_name: &str, room_code: String,
                                       aios_mgr: &AiosDBManager) -> DataCenterInstance {
...
async fn test_get_data_center_weld_attr() -> anyhow::Result<()> {
    ...
    let result = get_data_center_weld_attr(tee_node.into(), &owner_name, "".to_string(), &aios_mgr).await;
