...
pub async fn get_data_center_cap_attr(refno: PdmsElement, bran_name: &str, room_code: String, database: &ArDatabase, aios_mgr: &AiosDBManager) -> DataCenterInstance {
    ...
    DataCenterInstance {
        object_model_code: "ITEMAK".to_string(),
...
async fn test_get_data_center_cap_attr() -> anyhow::Result<()> {
    ...
    let result = get_data_center_cap_attr(tee_node.into(), &owner_name, "".to_string(),
