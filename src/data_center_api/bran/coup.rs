...
pub async fn get_data_center_coup_attr(refno: PdmsElement, bran_name: &str, room_code: String, database: &ArDatabase, aios_mgr: &AiosDBManager) -> DataCenterInstance {
    ...
    DataCenterInstance {
        object_model_code: "ITEMAG".to_string(),
...
#[tokio::test]
async fn test_get_data_center_coup_attr() -> anyhow::Result<()> {
    ...
    let result = get_data_center_coup_attr(tee_node.into(), &owner_name, "".to_string(),&database, &aios_mgr).await;
