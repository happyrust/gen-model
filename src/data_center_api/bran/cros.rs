...
pub async fn get_data_center_cros_attr(refno: PdmsElement,bran_name:&str,database:&ArDatabase,aios_mgr:&AiosDBManager) -> DataCenterInstance {
    ...
    DataCenterInstance {
        object_model_code: "ITEMAC".to_string(),
...
pub async fn get_dq_cros_data(refno: &PdmsElement, bran_name: &str, spre_name: &str,
                                                room_name: &str, ftub_paras: &Vec<f64>,pspec_name:&str,
                                                b_cover:bool, point_map: &HashMap<RefU64, InstPointMap>, aios_mgr: &AiosDBManager) -> anyhow::Result<DataCenterInstance> {
    ...
    data_center_attr.push(DataCenterAttr {
        ...
        value: AttrValue::AttrString("四通".to_string()).into(),
    });
    ...
    data_center_attr.push(DataCenterAttr {
        attribute_model_code: "PARTEJ27".to_string(),
        value: AttrValue::AttrString(pspec_name.to_string()).into(),
    });
    data_center_attr.push(DataCenterAttr {
        attribute_model_code: "PARTEJ28".to_string(),
    ...
    data_center_attr.push(DataCenterAttr {
        attribute_model_code: "PARTEJ30".to_string(),
        value: AttrValue::AttrVec3(arrive_pos).into(),
    });
    data_center_attr.push(DataCenterAttr {
        attribute_model_code: "PARTEJ31".to_string(),
...
#[tokio::test]
async fn test_get_data_center_cros_attr() -> anyhow::Result<()> {
    ...
    let result = get_data_center_cros_attr(tee_node.into(),&owner_name,&database,&aios_mgr).await;
