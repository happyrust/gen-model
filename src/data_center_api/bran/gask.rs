...
pub async fn get_data_center_gask_attr(refno: PdmsElement, bran_name: &str, room_code: String, database: &ArDatabase, aios_mgr: &AiosDBManager) -> DataCenterInstance {
...
#[tokio::test]
async fn test_get_data_center_gask_attr() -> anyhow::Result<()> {
    ...
    let result = get_data_center_gask_attr(tee_node.into(), &owner_name, "".to_string(),&database, &aios_mgr).await;
