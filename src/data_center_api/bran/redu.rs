...
pub async fn get_data_center_redu_attr(refno: PdmsElement,bran_name:&str,database:&ArDatabase,aios_mgr:&AiosDBManager) -> DataCenterInstance {
...
async fn test_get_data_center_redu_attr() -> anyhow::Result<()> {
    ...
    let result = get_data_center_redu_attr(tee_node.into(),&owner_name,&database,&aios_mgr).await;
