...
use crate::aql_api::pdms_room::{query_room_info_from_refno, query_room_name_from_refno_aql};
...
pub(crate) async fn auto_get_attr_from_metadata_excel(refno: RefU64, attr: &AttrMap, metadata: &Vec<DataCenterMetadata>, aios_mgr: &AiosDBManager) -> Vec<DataCenterAttr> {
    ...
    for data in metadata {
        if !data.function.starts_with("material()") { continue; }
        let filed = data.function.split(".").collect::<Vec<_>>();
        if filed.len() < 2 { continue; }
    ...
    for auto_data in metadata {
        ...
        if function.len() == 1 {
            let value = auto_get_attr_from_metadata_excel_single_function(aios_mgr, refno, function[0]).await
                .unwrap_or("".to_string());
...
async fn auto_get_attr_from_metadata_excel_single_function(aios_mgr: &AiosDBManager, refno: RefU64, function: &str) -> Option<String> {
    match function {
        "world_position()" => {
            let Ok(Some(position)) = aios_mgr.get_world_transform(refno).await else { return None; };
            Some(serde_json::to_string(&position.translation).unwrap_or("[0.0,0.0,0.0]".to_string()))
        }
        "orientation()" => {
            let Ok(Some(position)) = aios_mgr.get_world_transform(refno).await else { return None; };
...
fn auto_get_attr_from_metadata_excel_attr_function(function: &Vec<&str>, attr: &AttrMap, material_map: &DashMap<String, String>) -> Option<AttrVal> {
    if function.len() < 2 { return None; }
    match function[0] {
        "attr()" => {
            // 获取attr的某个属性
