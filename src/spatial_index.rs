...
impl Default for QueryOptions {
    fn default() -> Self {
        Self { types: vec![], tolerance: 0.0, limit: None, include_bbox: false, sort: None, exclude: vec![] }
    }
}

/// 空间查询后端统一接口
pub trait SpatialQueryBackend {
    fn query_intersect_hits(&self, query: &Aabb, opts: &QueryOptions) -> anyhow::Result<Vec<SpatialHit>>;
    fn query_contains_hits(&self, query: &Aabb, opts: &QueryOptions) -> anyhow::Result<Vec<SpatialHit>>;
    fn query_nearest_to_point(&self, point: Point3<f32>, k: usize, search_radius: Option<f32>, opts: &QueryOptions) -> anyhow::Result<Vec<SpatialHit>>;
    fn query_ray_hits(&self, origin: Point3<f32>, dir: Vector3<f32>, max_distance: f32, opts: &QueryOptions) -> anyhow::Result<Vec<SpatialHit>>;
}
...
impl SqliteSpatialIndex {
    ...
    pub fn is_enabled() -> bool {
        #[cfg(feature = "sqlite-index")]
        {
            ...
            if let Ok(built) = s.build() {
                ...
                built.get_bool("enable_sqlite_rtree").ok()
    ...
    pub fn query_intersect(&self, query: &Aabb) -> Result<Vec<RefU64>> {
        ...
        let ids = stmt.query_map(
            params![
                query.mins.x as f64, query.maxs.x as f64,
                query.mins.y as f64, query.maxs.y as f64,
                query.mins.z as f64, query.maxs.z as f64
            ],
    ...
    #[cfg(feature = "sqlite-index")]
    fn query_intersect_advanced(&self, query: &Aabb, opts: &QueryOptions) -> Result<Vec<SpatialHit>> {
        self.query_by_overlap(query, opts, false)
    }

    /// 包含查询：返回完全被 query AABB 包含的元素
    #[cfg(feature = "sqlite-index")]
    fn query_contains_advanced(&self, query: &Aabb, opts: &QueryOptions) -> Result<Vec<SpatialHit>> {
        self.query_by_overlap(query, opts, true)
    }

    /// 内部：统一构造 SQL 执行 overlap/contain 两类查询
    #[cfg(feature = "sqlite-index")]
    fn query_by_overlap(&self, query: &Aabb, opts: &QueryOptions, contains: bool) -> Result<Vec<SpatialHit>> {
        ...
        let mut sql = String::from("SELECT aabb_index.id, min_x, max_x, min_y, max_y, min_z, max_z FROM aabb_index ");
        let use_type_filter = !opts.types.is_empty();
        if use_type_filter { sql.push_str("JOIN items ON items.id = aabb_index.id "); }
        sql.push_str("WHERE ");
        if contains {
        ...
        let mut params: Vec<Box<dyn ToSql>> = vec![
            Box::new(minx), Box::new(maxx), Box::new(miny), Box::new(maxy), Box::new(minz), Box::new(maxz),
        ];

        if use_type_filter {
            sql.push_str("AND items.noun IN (");
            for i in 0..opts.types.len() { if i > 0 { sql.push_str(","); } sql.push_str("?"); }
            sql.push_str(") ");
            for t in &opts.types { params.push(Box::new(t.clone())); }
        }

        if !opts.exclude.is_empty() {
            sql.push_str("AND aabb_index.id NOT IN (");
            for i in 0..opts.exclude.len() { if i > 0 { sql.push_str(","); } sql.push_str("?"); }
            sql.push_str(") ");
            for e in &opts.exclude { params.push(Box::new(e.0 as i64)); }
        }

        if let Some(SortBy::Id(order)) = &opts.sort {
            sql.push_str("ORDER BY aabb_index.id ");
            match order { SortOrder::Asc => sql.push_str("ASC "), SortOrder::Desc => sql.push_str("DESC "), }
        }

        if let Some(limit) = opts.limit { sql.push_str("LIMIT ? "); params.push(Box::new(limit as i64)); }

        let mut stmt = conn.prepare(&sql)?;
        let rows = stmt.query_map(params_from_iter(params.iter().map(|p| &**p)), |row| {
            let id: i64 = row.get(0)?;
            let min_x: f64 = row.get(1)?; let max_x: f64 = row.get(2)?;
            let min_y: f64 = row.get(3)?; let max_y: f64 = row.get(4)?;
            let min_z: f64 = row.get(5)?; let max_z: f64 = row.get(6)?;
        ...
        for r in rows {
            let (id, mins, maxs) = r?;
            let aabb = Aabb::new([mins[0] as f32, mins[1] as f32, mins[2] as f32].into(), [maxs[0] as f32, maxs[1] as f32, maxs[2] as f32].into());
            hits.push(SpatialHit { refno: RefU64(id), bbox: if opts.include_bbox { Some(aabb) } else { None }, distance: None });
        }

        if let Some(SortBy::DistanceTo(p)) = &opts.sort {
            for h in hits.iter_mut() {
                let bb = if let Some(b) = &h.bbox { b.clone() } else { self.get_aabb(h.refno)?.unwrap_or_else(|| Aabb::new([0.0,0.0,0.0].into(), [0.0,0.0,0.0].into())) };
                h.distance = Some(distance_point_aabb(*p, &bb));
            }
            hits.sort_by(|a, b| a.distance.partial_cmp(&b.distance).unwrap_or(std::cmp::Ordering::Equal));
            if let Some(limit) = opts.limit { if hits.len() > limit { hits.truncate(limit); } }
        }
    ...
    /// 近邻查询（点）
    #[cfg(feature = "sqlite-index")]
    fn query_knn_point(&self, point: Point3<f32>, k: usize, search_radius: Option<f32>, opts: &QueryOptions) -> Result<Vec<SpatialHit>> {
        let mut radius = search_radius.unwrap_or(1.0_f32);
        let mut best: Vec<SpatialHit> = Vec::new();
        for _ in 0..10 {
            let q = Aabb::new((point - Vector3::new(radius, radius, radius)).into(), (point + Vector3::new(radius, radius, radius)).into());
            ...
            for h in hits.iter_mut() {
                if let Some(bb) = &h.bbox { h.distance = Some(distance_point_aabb(point, bb)); }
            }
            hits.sort_by(|a, b| a.distance.partial_cmp(&b.distance).unwrap_or(std::cmp::Ordering::Equal));
            if hits.len() >= k { best = hits.into_iter().take(k).collect(); break; }
            best = hits;
            radius *= 2.0;
        }
    ...
    /// 射线相交
    #[cfg(feature = "sqlite-index")]
    fn query_ray_internal(&self, origin: Point3<f32>, dir: Vector3<f32>, max_distance: f32, opts: &QueryOptions) -> Result<Vec<SpatialHit>> {
        let d = if dir.norm() > 0.0 { dir.normalize() } else { Vector3::new(1.0, 0.0, 0.0) };
        ...
        for h in hits.iter_mut() {
            if let Some(bb) = &h.bbox { h.distance = ray_aabb_toi(origin, d, bb, max_distance); }
        }
    ...
    pub fn delete_aabb(&self, refno: RefU64) -> Result<()> {
        ...
        tx.execute("DELETE FROM aabb_index WHERE id = ?1", params![refno.0 as i64])?;
    ...
    pub fn clear(&self) -> Result<()> {
        let conn = self.get_connection()?;
        conn.execute_batch(
            "DELETE FROM aabb_index; DELETE FROM items;"
        )?;
        Ok(())
    }
...
impl SpatialQueryBackend for SqliteSpatialIndex {
    ...
    fn query_nearest_to_point(&self, point: Point3<f32>, k: usize, search_radius: Option<f32>, opts: &QueryOptions) -> Result<Vec<SpatialHit>> {
        self.query_knn_point(point, k, search_radius, opts)
    }

    fn query_ray_hits(&self, origin: Point3<f32>, dir: Vector3<f32>, max_distance: f32, opts: &QueryOptions) -> Result<Vec<SpatialHit>> {
...
fn distance_point_aabb(p: Point3<f32>, bb: &Aabb) -> f32 {
    let dx = if p.x < bb.mins.x { bb.mins.x - p.x } else if p.x > bb.maxs.x { p.x - bb.maxs.x } else { 0.0 };
    let dy = if p.y < bb.mins.y { bb.mins.y - p.y } else if p.y > bb.maxs.y { p.y - bb.maxs.y } else { 0.0 };
    let dz = if p.z < bb.mins.z { bb.mins.z - p.z } else if p.z > bb.maxs.z { p.z - bb.maxs.z } else { 0.0 };
    (dx * dx + dy * dy + dz * dz).sqrt()
}

/// 射线与 AABB 最近命中距离（0..=max_distance），无命中返回 None
fn ray_aabb_toi(origin: Point3<f32>, dir: Vector3<f32>, bb: &Aabb, max_distance: f32) -> Option<f32> {
    ...
    if dir.x != 0.0 {
        ...
        if origin.x < bb.mins.x || origin.x > bb.maxs.x { return None; }
    }
    ...
    if dir.y != 0.0 {
        ...
        if origin.y < bb.mins.y || origin.y > bb.maxs.y { return None; }
    }
    ...
    if dir.z != 0.0 {
        ...
        if origin.z < bb.mins.z || origin.z > bb.maxs.z { return None; }
    }

    if tmax < 0.0 { return None; }
    let t = if tmin >= 0.0 { tmin } else { tmax };
    if t >= 0.0 && t <= max_distance { Some(t) } else { None }
}
