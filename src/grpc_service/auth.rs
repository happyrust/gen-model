...
impl AuthService {
    ...
    /// 生成JWT token
    pub fn generate_token(&self, user_id: &str, username: &str, roles: Vec<String>) -> ServiceResult<String> {
...
impl InputValidator {
    ...
    pub fn validate_task_id(task_id: &str) -> ServiceResult<()> {
        if task_id.is_empty() {
            return Err(ServiceError::InvalidRequest("Task ID cannot be empty".to_string()));
        }
        ...
        // 检查是否包含有害字符
        if task_id.contains(['<', '>', '"', '\'', '&']) {
            return Err(ServiceError::InvalidRequest("Task ID contains invalid characters".to_string()));
        }
    ...
    pub fn validate_mdb_name(mdb_name: &str) -> ServiceResult<()> {
        if mdb_name.is_empty() {
            return Err(ServiceError::InvalidRequest("MDB name cannot be empty".to_string()));
        }
        
        if mdb_name.len() > 200 {
            return Err(ServiceError::InvalidRequest("MDB name too long".to_string()));
        }
        
        // 检查路径遍历攻击
        if mdb_name.contains("..") || mdb_name.contains('/') {
            return Err(ServiceError::InvalidRequest("MDB name contains invalid path characters".to_string()));
        }
