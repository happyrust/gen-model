...
/// 获取所有需要计算的房间号
pub async fn query_all_need_compute_room_refno(dbno: Vec<i32>, room_type: &str, pool: &Pool<MySql>) -> anyhow::Result<Vec<RefU64>> {
    let mut refnos = vec![];
    let sql = gen_query_all_need_compute_room_refno_sql(dbno, room_type);
    let results = sqlx::query(&sql).fetch_all(&mut pool.acquire().await?).await?;
    for result in results {
        refnos.push(RefU64(result.get::<i64,_>("ID") as u64));
    }
    Ok(refnos)
}
...
fn gen_query_all_need_compute_room_refno_sql(dbnos: Vec<i32>, room_type: &str) -> String {
    ...
    sql.push_str(&format!("SELECT ID FROM {PDMS_ELEMENTS_TABLE} WHERE TYPE = '{}' AND NUMBDB IN ({})", room_type, dbno_str));
    sql
}
