...
impl DatabaseDiagnosticResult {
    ...
    fn update_overall_status(&mut self) {
        let has_critical = self.checks.iter().any(|c| matches!(c.status, DiagnosticStatus::Critical));
        let has_warning = self.checks.iter().any(|c| matches!(c.status, DiagnosticStatus::Warning));
...
async fn check_configuration(result: &mut DatabaseDiagnosticResult) {
    ...
    result.add_check(DiagnosticCheck {
        ...
        status,
        message,
        details: Some(format!("连接字符串: {}", db_option.get_version_db_conn_str())),
...
async fn check_surreal_cli(result: &mut DatabaseDiagnosticResult) {
    ...
    let cli_exists = match TokioCommand::new("which")
        .arg("surreal")
        .output()
        .await
    {
    ...
    let (status, message) = if cli_exists {
        ...
        (DiagnosticStatus::Healthy, format!("SurrealDB CLI 可用: {}", version_info))
    } else {
        (DiagnosticStatus::Critical, "SurrealDB CLI 未安装或不在 PATH 中".to_string())
    };
    ...
    if !cli_exists {
        result.add_recommendation("安装 SurrealDB CLI: curl -sSf https://install.surrealdb.com | sh".to_string());
    }
}
...
async fn check_database_functionality(result: &mut DatabaseDiagnosticResult) {
    ...
    let (status, message) = if functional {
        ...
        (DiagnosticStatus::Critical, format!("数据库功能异常: {}", error_msg.as_ref().unwrap_or(&"未知错误".to_string())))
    };
...
async fn check_process_status(result: &mut DatabaseDiagnosticResult) {
    ...
    let (status, message) = if pid_exists {
        (DiagnosticStatus::Healthy, "找到 SurrealDB 进程 PID 文件".to_string())
    } else {
        (DiagnosticStatus::Warning, "未找到 SurrealDB 进程 PID 文件".to_string())
    };
...
fn generate_recommendations(result: &mut DatabaseDiagnosticResult) {
    ...
    result.add_recommendation("手动测试连接: surreal sql --conn ws://localhost:8009 --user root --pass root".to_string());
}
