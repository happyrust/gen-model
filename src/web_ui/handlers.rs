...
pub async fn api_get_project(
    ...
    let sql = format!("SELECT *, id as id FROM type::thing('{}')", id_esc);
    match SUL_DB.query(sql).await {
        Ok(mut resp) => {
            let rows: Vec<serde_json::Value> = resp.take(0).unwrap_or_default();
...
pub async fn api_healthcheck_project(
    ...
    // 查询 health_url
    let id_esc = id.replace("'", "\\'");
    let get_sql = format!("SELECT health_url FROM type::thing('{}')", id_esc);
    ...
    let now = chrono::Utc::now().to_rfc3339();
    let sql = format!(
        "UPDATE type::thing('{}') MERGE {{ status: '{}', last_health_check: '{}', updated_at: '{}' }} RETURN AFTER",
...
pub async fn projects_health_scheduler() {
    ...
    loop {
        ...
        for row in rows {
            ...
            let id_esc = id.replace("'", "\\'");
            let update = format!(
                "UPDATE type::thing('{}') MERGE {{ status: '{}', last_health_check: '{}', updated_at: '{}' }}",
                id_esc, status_str, now, now
            );
...
async fn execute_real_task(state: AppState, task_id: String) {
    ...
    if config.gen_model {
        ...
        if let Err(e) = gen_all_geos_data(vec![], &db_option, None, config.target_sesno).await {
            let mut task_manager = state.task_manager.lock().await;
            if let Some(mut task) = task_manager.active_tasks.remove(&task_id) {
                task.status = TaskStatus::Failed;
                task.completed_at = Some(SystemTime::now());
