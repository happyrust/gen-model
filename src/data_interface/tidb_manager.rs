...
impl AiosDBManager {
    ...
    pub async fn init(db_option: &DbOption) -> anyhow::Result<Self> {
        ...
        let plin_cache_mgr = if let Some(pool) = project_map.get(&db_option.project_name) {
            cache_plin_plax(
                &project_map.get(&db_option.project_name).unwrap(),
    ...
    pub async fn resolve_expression_to_dir(
        &self,
        expr: &str,
        desi_refno: RefU64,
    ) -> anyhow::Result<Vec3> {
    ...
    pub async fn cache_pohe_geos(mgr: Arc<AiosDBManager>, project: &str) -> anyhow::Result<bool> {
        let pohe_refnos = mgr
            .get_refnos_by_types(project, &vec!["POHE"], Option::from(vec![1]))
            .await?;
    ...
    pub async fn cache_geos_data(
        ...
        for db_no in db_nos {
            ...
            let mut run_cache_loop = db_option.debug_refno_type.is_none();
            let mut run_cache_prim = db_option.debug_refno_type.is_none();
            let mut run_cache_cata = db_option.debug_refno_type.is_none();
            if db_option.debug_refno_type == Some("CATA".to_string()) {
                run_cache_cata = true;
            } else if db_option.debug_refno_type == Some("PRIM".to_string()) {
                run_cache_prim = true;
            } else if db_option.debug_refno_type == Some("LOOP".to_string()) {
                run_cache_loop = true;
            }
