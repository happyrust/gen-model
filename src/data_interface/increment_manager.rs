...
impl AiosDBManager {
    ...
    pub async fn execute_incr_update(
        ...
        for (path, (basic_info, sesno_range)) in increment_ranges_map {
            ...
            self.update_elements_to_database(&io, range_eles, basic_info.pdms_header.db_num)
                .await?;
    ...
    pub async fn execute_incr_update_single_sesno(
        ...
        // io.open().map_err(|e| anyhow::anyhow!("Failed to open PdmsIO: {}", e))?;
        // //需要找到最近的sesno
        // let start_sesno = io.get_nearest_large_sesno(start_sesno)
        //     .unwrap_or_default();
        // let cur_ses_data = io.get_ses_data(start_sesno as _)
        //     .map_err(|e| anyhow::anyhow!("Failed to get session data: {}", e))?
        //     .clone();
        // //收集哪些数据放生增删改
        // let mut eles_map = io
        //     .collect_increment_eles(None)?;
        ...
        //             // 将sync_map中的每个条目转换为适合eles_map的格式，以兼容类型系统
        //             // EleData转换为EleOperationDetail
        //             for (refno, ele_data) in sync_map {
        //                 eles_map.insert(refno, EleOperationDetail {
        //                     refno: ele_data.refno(),
        //                     children: ele_data.children.clone(),
        //                     operation: EleOperation::Modified, // 默认值，可能需要根据实际情况调整
        //                     attributes: ele_data.att_map().clone(),
        //                 });
        //             }
        //         }
        //     }
        ...
        // let refnos: Vec<(RefU64, Vec<RefU64>)> = resp.take(0)
        //     .map_err(|e| anyhow::anyhow!("Failed to extract data from response: {}", e))?;
        ...
        //     let mut attmap: NamedAttrMap = ele.attributes.clone();
        ...
        //         .await
        //         .map_err(|e| anyhow::anyhow!("Failed to backup deleted data: {}", e))?;
        // }
        ...
        //         .map_err(|e| anyhow::anyhow!("Failed to insert session data: {}", e))?;
        //     let sql = format!(
        ...
        //         .await
        //         .map_err(|e| anyhow::anyhow!("Failed to backup modified data: {}", e))?;
        ...
        //             if let Some(pe) = aios_core::get_pe(pe_data.owner).await
        //                 .map_err(|e| anyhow::anyhow!("Failed to get PE data: {}", e))? {
        ...
        //         }
        //         //保存 UDA 数据到数据库
        //         if let Some(uda_json) = attmap.gen_sur_json_uda(&["id"]) {
        ...
        //             );
        //             SUL_DB.query(sql).await
        //                 .map_err(|e| anyhow::anyhow!("Failed to upsert UDA data: {}", e))?;
        //         }
        //     }
        ...
        //             );
        //             let mut resp = SUL_DB.query(sql).await
        //                 .map_err(|e| anyhow::anyhow!("Failed to query delete owner relation: {}", e))?;
        //             let mut merged_children: Vec<RefU64> = resp.take(0)
        //                 .map_err(|e| anyhow::anyhow!("Failed to extract deleted owner relation data: {}", e))?;
        ...
        //                 } else {
        //                     insert_pos = merged_children.iter()
        //                         .position(|&o| o == child)
        //                         .ok_or_else(|| anyhow::anyhow!("Child not found in merged children"))?
        //                         + 1;
        //                 }
        //             }
        ...
        //                 // println!("{}", sql);
        //                 SUL_DB.query(sql).await
        //                     .map_err(|e| anyhow::anyhow!("Failed to insert owner relation: {}", e))?;
        //             }
        //         }
        //         SUL_DB.query("COMMIT TRANSACTION;").await
        ...
        //             continue;
        //         };
        //         let pe_data = ele.attributes.pe(dbnum);

        //         //处理几何体的筛选
        //         let noun = ele.attributes.get_type();
        //         let noun = noun.as_str();
        //         let refno_enum = refno.into();
        //         if PRIMITIVE_NOUN_NAMES.contains(&noun) {
        //             geo_update_log.prim_refnos.insert(refno_enum);
        ...
        //             if let Some(pe) = aios_core::get_pe(pe_data.owner).await
        //                 .map_err(|e| anyhow::anyhow!("Failed to get PE data for added item: {}", e))? {
        //                 final_check_geom_refnos.insert(pe.refno);
        //                 need_backup_geom_refnos.insert(pe.refno);
        //             }
        //         } else {
        ...
        //         if let Some(att_json) = ele.attributes.gen_sur_json() {
        //             SUL_DB
        //                 .query(format!(
        //                     "INSERT IGNORE INTO {} {}",
        //                     ele.attributes.get_type(),
        //                     att_json
        //                 ))
        //                 .await
        //                 .map_err(|e| anyhow::anyhow!("Failed to insert attribute data: {}", e))?;
        //         }
        //     }
        ...
        //                 .map_err(|e| anyhow::anyhow!("Failed to insert PE data: {}", e))?;
        ...
        //         println!("{}", sql);
        //         SUL_DB.query(sql).await
        //             .map_err(|e| anyhow::anyhow!("Failed to insert owner relation for changed map: {}", e))?;
        //     }
        // }
        // //几何体的处理
        // {
        ...
        //     )
        //     .await
        //     .map_err(|e| anyhow::anyhow!("Failed to backup geometry data: {}", e))?;
        ...
        //     }
        // }
        // let start_sesno = io.get_nearest_large_sesno(start_sesno + 1).unwrap_or_default();
        // Ok(start_sesno)
    }
    ...
    pub async fn update_elements_to_database(
        &self,
        io: &PdmsIO,
        range_eles: HashMap<RefU64, EleOperationDetail>,
        dbnum: i32,
    ) -> anyhow::Result<()> {
        // 分类元素
        // 收集所有SQL语句
        let mut sqls = Vec::with_capacity(range_eles.len());
        for (&refno, ele) in &range_eles {
            let sql = ele.to_surql(&refno.to_string());
            sqls.push(sql);
        }
