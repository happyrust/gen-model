...
pub async fn query_hole_detail_data_by_code(database: &Database, code: &str) -> anyhow::Result<Option<Vec<VirtualHoleGraphNode>>> {
    let aql = AqlQuery::new("FOR u IN @@collection
...
pub async fn query_embed_detail_data_by_code(database: &Database, code: &str) -> anyhow::Result<Option<Vec<VirtualEmbedGraphNode>>> {
    let aql = AqlQuery::new("FOR u IN @@collection
                                                FILTER u.REF==@code
                                                return unset(u , '_id','_rev')")
