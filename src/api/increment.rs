...
pub async fn query_key_data(key:&str,pool: &Pool<MySql>) -> anyhow::Result<Vec<IncrementDataSql>> {
    Ok(vec![])
}

fn gen_query_key_data_sql(key:&str) -> String {
    let mut sql = String::new();
    sql.push_str(&format!("SELECT * FROM {INCREMENT_DATA} WHERE ID = '{}'", key));
    sql
}
