...
async fn test_generate_xtk_for_refno_24383_92720() -> anyhow::Result<()> {
    ...
    match result {
        Ok(generation_result) => {
            ...
            println!("几何复用率: {:.2}%", generation_result.geometry_reuse_ratio * 100.0);
            println!("生成时间: {:.2}s", generation_result.generation_time.as_secs_f32());
...
async fn test_generate_xtk_batch() -> anyhow::Result<()> {
    ...
    println!("平均每个参考号: {:.2}s", total_time.as_secs_f32() / result.entity_count as f32);
...
async fn test_generate_xtk_different_quality() -> anyhow::Result<()> {
    ...
    for (config_name, xtk_config) in configs {
        ...
        let output_path = format!("{}/test_{}_{}.xkt", 
...
async fn test_performance_benchmark() -> anyhow::Result<()> {
    ...
    // 预热
    println!("预热运行...");
    let _ = generator.generate_xkt_from_refnos(
        vec![target_refno.clone()],
    ...
    for i in 1..=iterations {
        ...
        println!("迭代 {}: {:.2}s, 文件大小: {} KB", 
            i, iteration_time.as_secs_f32(), result.file_size / 1024);
    }
