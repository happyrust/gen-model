...
impl TestHelper {
    ...
    pub fn create_test_generators() -> Vec<(&'static str, XeokitXTKGenerator)> {
        vec![
            ("default", XeokitXTKGenerator::new(XTKGeneratorConfig::default())),
            ("high_performance", XeokitXTKGenerator::new(XTKGeneratorConfig::high_performance())),
            ("high_quality", XeokitXTKGenerator::new(XTKGeneratorConfig::high_quality())),
            ("debug", XeokitXTKGenerator::new(XTKGeneratorConfig::debug())),
        ]
    }
...
impl XTKFileInfo {
    ...
    pub async fn show_file_info(file_path: &str) -> anyhow::Result<()> {
        ...
        println!("文件大小: {} KB ({} bytes)", metadata.len() / 1024, metadata.len());
        
        if file_content.len() >= 4 {
            let version = u32::from_le_bytes([
                file_content[0], file_content[1], 
                file_content[2], file_content[3]
            ]);
