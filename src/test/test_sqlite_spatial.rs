...
mod tests {
    ...
    fn test_sqlite_spatial_basic() {
        ...
        let spatial_index = SqliteSpatialIndex::new(&db_path)
            .expect("Failed to create spatial index");
        println!("✅ SQLite 表结构已自动初始化");
        
        // 2. 创建测试数据
        let test_aabbs = vec![
            (RefU64(1112_00001), Aabb::new(Point3::new(0.0, 0.0, 0.0), Point3::new(10.0, 10.0, 10.0))),
            (RefU64(1112_00002), Aabb::new(Point3::new(5.0, 5.0, 5.0), Point3::new(15.0, 15.0, 15.0))),
            (RefU64(1112_00003), Aabb::new(Point3::new(20.0, 20.0, 20.0), Point3::new(30.0, 30.0, 30.0))),
            (RefU64(1112_00004), Aabb::new(Point3::new(-10.0, -10.0, -10.0), Point3::new(0.0, 0.0, 0.0))),
            (RefU64(1112_00005), Aabb::new(Point3::new(100.0, 100.0, 100.0), Point3::new(110.0, 110.0, 110.0))),
        ];
        ...
        for (refno, aabb) in &test_aabbs {
            if let Err(e) = spatial_index.insert_aabb(*refno, aabb, Some(&format!("test_{}", refno.0))) {
                panic!("❌ 插入数据失败: {}", e);
            }
        }
        ...
        for (refno, expected_aabb) in &test_aabbs {
            match spatial_index.get_aabb(*refno) {
                Ok(Some(aabb)) => {
                    assert_eq!(aabb.mins, expected_aabb.mins, "RefNo {} mins 不匹配", refno.0);
                    assert_eq!(aabb.maxs, expected_aabb.maxs, "RefNo {} maxs 不匹配", refno.0);
        ...
        println!("  查询框: min({:.1}, {:.1}, {:.1}) max({:.1}, {:.1}, {:.1})",
                 query_bbox.mins.x, query_bbox.mins.y, query_bbox.mins.z,
                 query_bbox.maxs.x, query_bbox.maxs.y, query_bbox.maxs.z);
        ...
        match spatial_index.query_intersect(&query_bbox) {
            Ok(results) => {
                ...
                assert!(!results.contains(&RefU64(1112_00003)), "不应该包含 1112_00003");
                assert!(!results.contains(&RefU64(1112_00005)), "不应该包含 1112_00005");
            }
            Err(e) => {
                panic!("❌ 空间查询失败: {}", e);
            }
        ...
        match spatial_index.query_intersect(&exact_bbox) {
            Ok(results) => {
                ...
                assert!(results.contains(&RefU64(1112_00001)), "应该包含 1112_00001（相交）");
                assert!(results.contains(&RefU64(1112_00002)), "应该包含 1112_00002（完全匹配）");
            }
            Err(e) => {
                panic!("❌ 精确查询失败: {}", e);
            }
    ...
    fn test_sqlite_spatial_performance() {
        ...
        println!("  - 平均返回: {} 个结果", total_results / query_count as usize);
        println!("  - 查询速度: {:.0} 查询/秒", 
