...
use crate::spatial_index::{SqliteSpatialIndex, SharedSpatialIndex};
...
fn init_db_7999_test_data() -> Vec<TrayTestData> {
    vec![
        ...
        TrayTestData {
            ...
            bbox: Aabb::new(
                Point3::new(115.0, 0.0, 20.2),
                Point3::new(115.4, 5.0, 20.4),
            ),
            element_type: "SUPPO".to_string(),
        ...
        TrayTestData {
            ...
            bbox: Aabb::new(
                Point3::new(95.0, 8.0, 19.0),
                Point3::new(125.0, 8.5, 22.0),
            ),
...
async fn test_sctn_spatial_query(index: &SqliteSpatialIndex) -> Result<()> {
    ...
    // 获取目标SCTN的包围盒
    let target_bbox = index.get_aabb(target_refno)?
...
async fn test_sctn_contact_detection(test_data: &[TrayTestData]) -> Result<()> {
    ...
    // 找到目标SCTN数据
    let target_data = test_data.iter()
    ...
    let target_sctn = CableTraySection {
        ...
        support_points: vec![
            Point3::new(102.2, 5.0, 20.3),
            Point3::new(107.2, 5.0, 20.3),
        ],
    ...
    println!("目标SCTN: {} ({})", target_refno.0, target_data.element_name);
...
async fn test_support_detection(test_data: &[TrayTestData]) -> Result<()> {
    ...
    // 检测支撑关系
    let supports = detector.detect_support_relationships(&target_sctn, 10.0).await?;
    ...
    let sctns: Vec<CableTraySection> = test_data
        ...
        .map(|td| {
            ...
            CableTraySection {
                ...
                centerline: vec![],
                width: 0.6,
                height: if td.element_name.contains("Vertical") { 3.0 } else { 0.3 },
