...
async fn test_export_water_calculation_stp_1() -> anyhow::Result<()> {
    ...
    export_models_map.insert(RefU64::from_str("17496/106424").unwrap(), "STWALL 1".to_string());
    stp_packet.export_models_map = export_models_map;
    //所有墙与孔洞的map
    let mut walls_map = HashMap::new();
    walls_map.insert(RefU64::from_str("17496/106424").unwrap(), vec![FloodingHole {
...
async fn test_export_water_calculation_stp_3() -> anyhow::Result<()> {
    ...
    //导出模型列表
    let mut export_models_map = HashMap::new();
    export_models_map.insert(RefU64::from_str("25688/8143").unwrap(), "STWALL 1".to_string());
    stp_packet.export_models_map = export_models_map;
    //所有墙与孔洞的map
    let mut walls_map = HashMap::new();
    walls_map.insert(RefU64::from_str("25688/8143").unwrap(), vec![FloodingHole {
...
async fn test_export_water_calculation_stp_4() -> anyhow::Result<()> {
    ...
    //所有墙与孔洞的map
    let mut walls_map = HashMap::new();
    walls_map.insert(RefU64::from_str("25688/19684").unwrap(), vec![FloodingHole {
