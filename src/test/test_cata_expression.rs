...
use regex::Regex;
use crate::cata::resolve_helper::{eval_str_to_f32, intern_eval_str_to_f64, parse_str_axis_to_vec3};
...
fn test_parse_param_with_of_operator() {
    ...
    // 是提前准备，还是在使用的时候去获取
    let r = intern_eval_str_to_f64::<AiosDBManager>(input_exp, &context, Some(&interface));
    dbg!(&r);
...
fn parse_3_axis() {
    ...
    let r = intern_eval_str_to_f64::<AiosDBManager>(str, &context, None);
    dbg!(r);
}
...
fn test_math_exp() {
    ...
    dbg!(intern_eval_str_to_f64::<AiosDBManager>(expr, &context, None)).expect("TODO: panic message");
}
