    ...
    for refno in refnos {
        ...
        if let Some(RefU64Type(spre)) = attr.get_val("SPRE") {
            ...
            if let Some(RefU64Type(catr)) = spre_attr.get_val("CATR") {
                ...
                type_attrs.entry(catr_attr.get_type().to_string()).or_insert_with(Vec::new).push(catr_attr);
            }
            if let Some(RefU64Type(detr)) = spre_attr.get_val("DETR") {
                all_refnos.push(*detr);
                let detr_attr = aios_mgr.get_attr(*detr).await?;
                type_attrs.entry(detr_attr.get_type().to_string()).or_insert_with(Vec::new).push(detr_attr);
            }
            type_attrs.entry(spre_attr.get_type().to_string()).or_insert_with(Vec::new).push(spre_attr);
        }
        type_attrs.entry(attr.get_type().to_string()).or_insert_with(Vec::new).push(attr);
    }
    dbg!(&all_refnos);
    dbg!(&all_refnos.len());
    // 将 attr 导出
