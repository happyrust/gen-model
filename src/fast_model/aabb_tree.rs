...
pub async fn manual_update_aabbs(replace_exist: bool) -> anyhow::Result<()> {
    ...
    loop {
        // 分批查询 inst_relate 的键
        let sql = format!("SELECT value in.id AS refno FROM inst_relate LIMIT {QUERY_CHUNK_SIZE} START {offset}");
        ...
        // 处理这批 refnos
        if !refnos.is_empty() {
            println!("Processing batch of {} inst_relate entries (offset: {})", refnos.len(), offset);
    ...
    println!("Successfully updated AABBs for all {} inst_relate entries", total_processed);
    Ok(())
}
