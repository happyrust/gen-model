...
use crate::data_to_file::{get_last_page, get_refno_position_in_page};
...
impl ClaimPage {
    ...
    pub fn convert_new_claim_page(self, input: &[u8]) -> Option<Vec<u8>> {
        ...
        let last_claim_page = get_last_page(input, self.refno, CLAIM_PAGE_ONE);
        if last_claim_page.is_none() { return None; }
        let mut last_claim_page = last_claim_page.unwrap();

        // 找到该claim_page最上面的参考号
        let first_refno = get_claim_page_first_refno(&last_claim_page);
        let claim_page_two = get_last_page(input, first_refno, CLAIM_PAGE_TWO);
        if claim_page_two.is_none() { return None;}
        let mut claim_page_two = claim_page_two.unwrap();
        ...
        claim_page_two.splice(refno_position + 8 .. refno_position + 12,(self.index_page_num + 1).to_be_bytes().to_vec());
