...
- 2026-08-05 增补（issue #7 审核后加固：**两条分支对空间树的依赖方向是相反的**）——
  - 这是第 6 条「两树合一」之后才成立、此前没有写下来的不变量：整间分支拿**面板
    ...
    但消费端此前没有对应的防线。
  - **空候选集门禁**：新增 `PanelTreeCoverage`（`room_model.rs`），一轮 drain 量
    一次「在册面板里有多少块真的在树上」并整轮复用（与 `load_room_panel_map` 同
    一个按轮复用的理由）。树上有在册面板 → 捞不到候选就是真的不在任何房间里，
    照旧收敛成空集，与全量一致；一块都没有 → 上抛，任务保留重试，轮开头另打一行
    说明。口径与第 8 条既有的「网格判不了就不改写」完全一致——**判不了不等于不在
    里面**，而这条路后面紧跟着的就是删该构件全部入边。
  - 同轮补掉另外三处静默：构件查不到实例、世界包围盒不可用各打一行日志（两条路
    本不该走到，元素任务的入队条件就是「包围盒确实变了」）；`enqueue_room_recalc`
    在 `gen_spatial_tree` 关闭且确实有变更时按进程告警一次，否则那条路上队列里从此
    不会出现任何 `room_recalc` 行、房间轮每轮早退、泳道空着，现场只看得到「模型
    动了、房间号不动」。
  - 三条回退即红的测试：覆盖率谓词真值表、覆盖率门禁必须排在那条写入之前（顺序反
    了等于没判，DELETE 已经把边带走）、开关关闭必须留下痕迹。
    `cargo test --lib --features http_api` 340 通过 / 0 失败。
  - **仍未闭合**：这只是让故障可见，不是消除它。真正的根治是让元素分支不依赖空间树
    ——在册面板只有百来块，候选完全可以从库里的面板包围盒算，一次查询整轮复用。
    记为后续项。issue #7 本身仍开着，见
