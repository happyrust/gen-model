...
启动时先读取数据库表定义，再幂等创建缺失的增量状态表。若 `dbnum_watermark` 整张表原本
不存在，说明这是在已有业务数据上兼容启用增量：此时按 `pe` 中每个 dbnum 已持久化数据的
最大 `sesno` 建立 `applied_sesno`，以当前数据库实际内容为基线，而不是拿现场文件最新会话
冒充已应用水位。若水位表原本存在，则已有 `applied_sesno` 优先；旧行只有 `sesno` 时固化该
值；没有专用水位行但 `dbnum_info_table` 有历史统计时，才以该 dbnum 的最大统计 `sesno`
回填。所有迁移只填空值，绝不覆盖已建立的 `applied_sesno`。仍无法得到历史水位的 DESI
不得猜测范围，必须先由全量项目生成建立明确基线，再允许增量应用。
