...
启动时先读取数据库表定义与现有水位行数，再幂等创建缺失的增量状态表。若 `dbnum_watermark`
整张表原本不存在，**或表虽已建立但还没有任何水位行**（上一次启动在建表之后、播种完成之前
中断留下的空表），说明这是在已有业务数据上兼容启用增量：此时按 `pe` 中每个 dbnum 已持久化
数据的最大 `sesno` 建立 `applied_sesno`，以当前数据库实际内容为基线，而不是拿现场文件最新
会话冒充已应用水位。空表没有任何已建立水位需要保护，与缺表同判不会有损失。若水位表已有
行，则已有 `applied_sesno` 优先；旧行只有 `sesno` 时固化该值；没有专用水位行但
`dbnum_info_table` 有历史统计时，才以该 dbnum 的最大统计 `sesno` 回填。所有迁移只填空值，
绝不覆盖已建立的 `applied_sesno`。播种走 `pe` 源时对全表聚合，大库上耗时较长，启动日志会
打出开始与完成（含耗时）两行。仍无法得到历史水位的 DESI 不得猜测范围，必须先由全量项目
生成建立明确基线，再允许增量应用。
