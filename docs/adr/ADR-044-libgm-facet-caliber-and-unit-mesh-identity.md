...
2. **所有复用曲面的单位网格身份带上显式 caliber。** `hash_unit_mesh_params()` 对普通柱、
   球、Dish、CTorus、RTorus、同心 LSnout 混入从原始真实尺寸算出的 `FacetCaliber`；
   `gen_unit_shape()` 必须携带同一个值。SSCL 与偏心 Snout 仍按整参数身份生成。旧的单位参数
   缺失 caliber 时响亮报告「需要整库重建」，不得再从单位半径或写死段数反推。
...
- 身份键升级与目标库 ADR-021 整库重建在同一维护窗口原子切换；不保留新旧 `.mesh` 双轨。
- T053 是 T041 的硬前置，容量按 8009/7997 两库较大的新身份数并加 25% 余量估算。
- 球、SSCL、多面体、YOFF Snout 必须各有 dabacon 现场样本；缺任一类即停止 T031。
- OCC 在退役门全绿后从主仓、vendor API、Python、CI 和发布产物彻底删除，不保留参照 feature。
- RVM 与双版本 libgm 逆向证据取代 OCC 作为独立参照；部署回滚使用预先归档的过渡二进制。
