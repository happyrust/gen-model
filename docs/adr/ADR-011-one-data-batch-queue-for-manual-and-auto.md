...
- **单 worker 已落地**（`batch_worker.rs`）：无条件 spawn、不分 sync_live（`ensure_batch_worker`
  幂等守卫防双消费者）；出队冻结 → 执行 → 终态；每批次内按旧手动顺序补 SYST 派生入账、
  副作用补偿、非 regen drain 与本批交付单元生成（progress 事件挂在批次任务上）。
...
- **§8（房间在队列跑空时收一轮）已落地**：worker 空闲轮先消化积压（副作用 +
  `drain_data_phases`），再把房间收敛包成一条 `room_recalc` 任务（有目标才建行）。
...
2. **两条路径合流**：`async_watch` 从「发现即处理」改为「发现即入队」，手动执行同样
   只入队。一条队列、一个消费者、一套可见性；`sync_live = true` 时手动更新不再 422。
...
7. **完成判据是两段都成**。水位推了但交付单元有失败 = 部分完成，该 dbnum 行上常驻
   「欠 N 个单元」；欠的那些走既有的待重试单元通道并入下一批。
8. **房间重算在队列跑空时收一轮**，不跟在每个批次后面。ADR-010 §7 已经把它钉死在
   「几何与 AABB 都已落定」之后，并行是错的；而 `drain_rooms` 开头的
   `load_room_panel_map` 是按轮加载不是按任务加载，每批跑一遍等于把这笔开销乘上批次数。
   房间是最终一致，滞后是 ADR-010 §1 自觉接受的代价。
...
11. **任务记录分层保留**：`queued` 与 `running` 永不剔除；每个 dbnum 保留最近一条终态；
    ...
    「欠 N 个单元」不依赖任务历史，它走持久的 `manual_model_pending` 表
    （`GET /update/pending-units`），历史滚掉不影响它。
12. **删掉 HTTP 层的单飞预检**，执行请求一律 202 入队。四种 kind 里没有一对真需要
    409 挡住：数据批次由单 worker 天然串行，预览只读，房间只在队列跑空时跑，
    按需生成早有 per-生成根锁。互斥是调度器的性质，在 HTTP 层再写一遍只会产生假冲突。
