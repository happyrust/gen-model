...
状态：已接受，完整性容忍规则由 ADR-037 修订（2026-08-19）

> 2026-08-19 完整性修订：ADR-037 取代本文关于“任意终稿解析失败、已选中子页
> 读取失败或层级异常可 warning 后继续”的规则。现在只有最小身份明确为代码白名单
> `MNUM` 的终稿可跳过；其他终稿失败、已选中 child 的读取/解析失败、层级未下降均
> 使窗口失败。计数字段继续保留用于结构性不可达分支和 MNUM 诊断，不代表可提交
> 不完整窗口。

> 2026-08-19 补充：ADR-036 发现 E3D 删除后旧物理记录仍可能被索引遍历触达，
> 因此仅对父成员表净减少候选增加“目标成员存活”收口。单一净窗口和双根差分的
> 普通增改候选地位不变；删除候选的完整规则以 ADR-036 为准。
