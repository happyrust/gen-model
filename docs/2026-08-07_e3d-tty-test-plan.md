...
| 2026-08-14 08:50 | TTY × 纯文件解析器 | FTUB `24384/23262` POS apply/restore；`parse.net_window` 语义净窗口 | PASS；会话 218→219→220，POS.U 2900→3400→2900，合并窗口目标业务变化净零；剩余 BRAN.CACHID 为 E3D 保存元数据；rollback 已验证 | `output/e3d-tty-net-window/20260814-085034/`；`docs/evidence/2026-08-14-e3d-tty-net-window.md` | 首跑发现 `parse.net_changes` 只是记录位置触达集，新增 `parse.net_window` 复用生产属性合成器并固化 `Test-TtyNetWindow.ps1` |
