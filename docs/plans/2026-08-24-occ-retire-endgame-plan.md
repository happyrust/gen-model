...
- **V5（整库重建，需排窗口）**：改身份 = 整库重建（ADR-044 决策 6）。
  `.mesh` 份数按 T053 定稿的五类口径（2026-08-25，库 A）：**392 → 474（+82）**，
  其中柱一行 1→44。~~按 T045 数据预估 1→7（8009）/ 1→37（7997）~~——那两个数
  只数了柱，且只数了**正体**（`insts_flat` 是带 `geo_type = 'Pos'` 的读侧投影，
  而负体走同一张 `geo_hash → .mesh` 表）。证据
  `docs/evidence/2026-08-25-t053-segment-identity-scope.md`。
  **决策点 D1（已拍板，见决策点汇总）**：不设独立窗口，V1–V4 合入同批立即重建，
  混合期目标为零；V3 合入后到重建完成前柱/球增量走新键写新行，双轨上限 1 个工作日。
...
- **L8（T045a）** 两个库都没有球 / SSCL / 多面体。
  **决策点 D2（已拍板，见决策点汇总）**：不找第三个库，在 E3D 测试工程造最小样本
  随 R4 导 RVM；造不出才注记「未经现场验证」收口——不得默认它们跟圆柱一样安全。
...
## 决策点汇总（2026-08-24 已拍板；执行中发现前提变化再回开）

| # | 事项 | 拍板 |
|---|---|---|
| D1 | T041 整库重建的窗口与范围 | **不设独立窗口**：V1–V4 合成一批，rev bump 合入后同批立即重建，混合期目标为零；只有某库正在服务增量时才容忍双轨，上限 1 个工作日。范围 = 仍要继续吃增量或充当 RVM 基准 / live 测试数据源的库（7997 副本母本 + 8009 系副本），重建按 T041+T053 定稿的**全部五类复用曲面原语**新键走；已损坏的 `.surreal/ams-8009` 照旧不修不建。**2026-08-25 范围已数完**（`docs/evidence/2026-08-25-t053-segment-identity-scope.md`）：库 A 五类合计 **392 行 → 474 行（+82）**，柱自己 **1 → 44**。旧估 37 只算了正体（`insts_flat` 带 `geo_type = 'Pos'` 的读侧投影），而负体也吃 `.mesh`；Pos 一侧重算恰好也是 37。量级与旧估同档，本决策不回开。 |
| D2 | 球 / SSCL / 多面体（+ 偏心 Snout 补样）两库无样本 | **造样本优先，不再找第三个库**：R4 基准本来就要从 E3D 导 RVM，同一环境顺手在测试工程造球 / SSCL / 多面体最小件（`scripts/e3d/` 的 `.mac` 宏是现成路子），偏心 Snout 若库 B 运行态复不起来也并进这批。E3D 侧造不出（环境/权限）才降级为本计划注记「未经现场验证」收口，且降级的前提是三类的权威规则已被单测钉住（球 n 与恒 n/2、SSCL 折叠表均出自反编译；多面体全平面、无段数问题，风险最低）。 |
| D3 | RVM 基准重建选库 | **单跑库 A（7997 副本）为主基准**：碟 17 行（15 椭圆 + 2 球碟）、两种环面 262 行（含 rins=0 现役喇叭环面）、8,094 行规模，样本面最广。两条例外：偏心 Snout 门（T050 门 b）用库 B 运行态（能复起）或 D2 造样本；球随 D2。**不双跑**——双基准口径混乱、成本翻倍，库 B 的增量差（快照少 6 行）也说明它不适合当可复现基准。 |
| D4 | 双会话分工 | 收官计划全程（specs/009 + vendor `prim_geo`/`shape` 区）**归本会话**；另一会话守净窗口 / 并行根 / insts-flat / 空间树线（ADR-031/041/043/045、specs/023–026），vendor `wire.rs` 归它（现压着它 156/73 行未提交改动）。硬边界两条：本会话不碰 `wire.rs`；Phase V 推上游前对表一次，wire 线未收口就按文件拆分提交、不整仓提交。共享 target 的编译互锁（pest_derive LNK1104 先例）：跑全量前先查在飞的 cargo/rustc。 |
...
3. **混合期身份**：V3 合入后至重建完成前，新增量走新键、存量旧键——查询按
   `geo_hash` 直址不受影响，但 `.mesh` 份数巡检会看到双轨（D1 已拍板：同批重建、
   混合期目标为零，双轨上限 1 个工作日）。
