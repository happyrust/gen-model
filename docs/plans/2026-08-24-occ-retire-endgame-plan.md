...
| # | 事项 | 拍板 |
|---|---|---|
| D1 | T041 整库重建的窗口与范围 | **不设独立窗口**：V1–V4 合成一批，rev bump 合入后同批立即重建，混合期目标为零；只有某库正在服务增量时才容忍双轨，上限 1 个工作日。范围 = 仍要继续吃增量或充当 RVM 基准 / live 测试数据源的库（7997 副本母本 + 8009 系副本），重建按 T041+T053 定稿的**全部五类复用曲面原语**新键走（1→7 / 1→37 是柱的旧估，五类要重估）；已损坏的 `.surreal/ams-8009` 照旧不修不建。 |
