...
- **W2.2 在任何 staging 模型修改前一次性持锁**（`batch_worker.rs::execute_frozen_batch_body`）
  ...
  - 按需生成不改：`on_demand_model` 走的 `generation_root_lock` 是同一把锁，
    命中被窗口持有的根时**立即回 409 conflict**——`try_generation_root` 是 `try_lock`，
    不排队。别再写成「等待」：一个 HTTP 请求挂住整个窗口的时长不是可接受的语义。
    拒绝面积随本项一起扩大，见 §4 R-C 与 ADR-017 结果 / 约束。
...
- **R-C 统一根锁扩大拒绝面**：容器 Transform 展开后代生成根，可能一次持有上百把锁，
  且持锁贯穿整个窗口（解析 + 生成 + 提交 + 空间收敛）。按需生成命中时**一律立即拒绝**
  （409 conflict，不排队，见 W2.2），所以现场可见的是「窗口进行期间这批根成片点不开」。
  缓解：ADR-017 结果/约束已按「拒绝」这一种口径写死；回执注明窗口进行中；
  排序获取避免死锁。
