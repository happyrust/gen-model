...
.venv\Scripts\maturin.exe develop --release  # 跑生成类操作前（布尔运算 debug 慢）
```

- 绑定 crate 是仓内 workspace 成员，与主 crate 共享 `Cargo.lock` / `target/` /
  `[patch]` vendor 重定向（`Toggle-LocalDeps.ps1` 同样生效），原生依赖只编一次。
...
- 实测（2026-08-12，共享 `D:\Rust\target` 缓存已热）：`develop --release` 从改动
  ...
  耗时也几乎一样（12.4s vs 12.9s）——这套用例是进程启动与建库主导的，布尔运算
  的 debug/release 差距要跑真实生成（`model.gen*` / 整库）才看得出来。
- 包装层（`pysrc/aios_db/__init__.py`）在导入扩展前把 PATH 上所有目录注册进 DLL
  搜索路径，让搜索面与主程序 exe 一致，正常情况无需手工干预。
...
（`.github/workflows/windows-tests.yml` 的 `python-bindings` job：`maturin build`
→ 装 wheel → smoke-import → 跑这一档 → 传 wheel artifact）。
