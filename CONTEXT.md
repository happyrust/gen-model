...
**数据支撑 (Data Backing)**：
应用水位的对偶约束：`applied_sesno > 0` 时该 `dbnum` 必须在 `pe` 里存在至少一行数据。判定用存在性查询而不是计数，只在「基线还是增量」的路由决策上使用、不进入队门；查询失败必须上浮为批次失败，不得吞成「有数据」或「没有数据」中的任何一种默认值。
_Avoid_: 行数校验、pe 计数、数据存在性（泛指时）

**幽灵水位 (Ghost Watermark)**：
`applied_sesno > 0` 而该 `dbnum` 在 `pe` 里零行的状态——应用水位声称落库过，库里却没有数据支撑（多由 `dbnum_info_table` 播种回填或历史解析中断留下）。处置是路由到首次导入重建基线，不从水位往后接增量；它不是文件异常，不进 `FileAnomaly`，也不删任何数据。
