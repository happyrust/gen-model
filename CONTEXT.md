...
持久化保存的独立模型任务：按需生成的保护记录、跨窗口派生工作（目录反向传播、延迟级联恢复）与房间任务积压。稳态增量窗口内部的解析与生成工作不入表——它们随提交单元整体成败（见「窗口阻断」）。任务以 `(action, target_refno)` 为唯一身份，`dbnum` 是由 Ref0 库归属得到的路由与校验字段，不参与任务身份；同一动作与目标只保留一条最新任务，每次确认有新工作时递增队列内部 `revision`，消费以 revision 收口。
...
常驻内存数据库实例中按提交单元建立的独立 database；承载该单元的暂存工作集，跨重试保留（重试只重跑失败根），提交或废弃后整库删除。进程崩溃即消失，由应用水位驱动整窗口重算。
_Avoid_: 内存库（泛指）、镜像库
