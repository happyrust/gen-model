...
import { Loader2, Download, Eye, Package, CheckCircle, XCircle, Info } from "lucide-react"
...
export function XKTGenerator() {
  ...
  const generateXKT = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8080/api/xkt/generate", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          dbno: parseInt(params.dbno),
      ...
      if (result.success) {
        ...
        // 获取文件大小
        try {
          const fileResponse = await fetch(`http://localhost:8080${result.url}`)
          if (fileResponse.ok) {
      ...
      toast.error(error.message || "生成XKT文件失败")
    } finally {
