...
use aios_database::grpc_service::sctn_raycast_detector::{
    AdvancedRaycastAnalyzer, SctnRaycastDetector, SupportCandidate,
};
...
async fn main() -> Result<()> {
    ...
    let sctn = {
        ...
        ex.extract_sctn_geometry(target)
            .await
            .unwrap_or_else(|_| reconstruct_sctn_from_aabb(target, &tb))
    };
    #[cfg(not(feature = "grpc"))]
    ...
    for r in neigh {
        if let Some(b) = index.get_aabb(r)? {
            #[cfg(feature = "grpc")]
            let typ = if let Some(ref dbm) = dbm_opt {
                dbm.get_type_name(r).await
            } else {
                "".into()
            };
            #[cfg(not(feature = "grpc"))]
    ...
    let supports = detector
        .detect_supports_by_raycast(&sctn, &candidates)
        .await?;
    ...
    for s in &supports {
        println!(
            "  ✓ 支撑 {} 于 ({:.3},{:.3},{:.3})",
            s.support.0, s.contact_point.x, s.contact_point.y, s.contact_point.z
        );
    }
    ...
    println!(
        "\n跨度统计: 数量={} 最大={:.3} 最小={:.3} 平均={:.3}",
        span.num_supports, span.max_span, span.min_span, span.avg_span
    );
