...
async fn main() -> Result<()> {
    ...
    if let Some(bbox) = sqlite_index.get_aabb(target_refno)? {
        ...
        for refno in nearby.iter().take(20) {  // 只检查前20个
            ...
            if let Some(contact) = detector.check_detailed_contact(&sctn, candidate, true)? {
                ...
                match contact.contact_type {
                    ContactType::Surface | ContactType::Edge | ContactType::Point => {
                        contact_count += 1;
                        println!("  ⚠️ 接触: {} 距离 {:.3}m", candidate.refno.0, contact.distance);
                    }
                    ContactType::Penetration => {
                        contact_count += 1;
                        println!("  ❌ 穿透: {} 深度 {:.3}m", candidate.refno.0, contact.penetration_depth);
                    }
...
fn load_db_config() -> Result<DbConfig> {
    ...
    Ok(DbConfig {
        host: settings.get_string("database.host").unwrap_or_else(|_| "localhost".to_string()),
        port: settings.get_int("database.port").unwrap_or(3306) as u16,
        user: settings.get_string("database.user").unwrap_or_else(|_| "root".to_string()),
        password: settings.get_string("database.password").unwrap_or_default(),
        database: settings.get_string("database.name").unwrap_or_else(|_| "aios".to_string()),
    })
}
