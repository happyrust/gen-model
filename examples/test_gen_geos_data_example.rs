...
async fn main() -> anyhow::Result<()> {
    ...
    db_option.debug_refno_types = vec!["PRIM".to_string(), "LOOP".to_string(), "CATA".to_string()];
    println!("   ✓ 测试配置完成\n");
    ...
    match test_gen_geos_data_from_database(dbno, &refno_types, max_refnos, &db_option).await {
        Ok(stats) => {
            ...
            println!(
                "     处理速度: {:.2} 参考号/秒",
                stats.performance_metrics.refnos_per_second
            );
            println!(
                "     生成速度: {:.2} 实例/秒",
                stats.performance_metrics.instances_per_second
            );
        }
        Err(e) => {
            println!("   ❌ 数据库测试失败: {}", e);
...
fn analyze_batch_results(all_stats: &[aios_database::test::GenGeosDataPerformanceStats]) {
    ...
    for (index, stats) in all_stats.iter().enumerate() {
        ...
        println!(
            "       - 平均处理时间: {:.2}ms/参考号",
            stats.performance_metrics.avg_time_per_refno_ms
        );
        println!(
            "       - 状态: {}",
            if stats.success { "成功" } else { "失败" }
        );
    }
    ...
    if successful_stats.len() >= 2 {
        ...
        let last_speed = successful_stats
            .last()
            .unwrap()
            .performance_metrics
            .refnos_per_second;
    ...
    // 效率评估
    let avg_speed: f64 = successful_stats
        .iter()
