...
fn main() {
    ...
    for (item, distance) in &nearby_items {
        println!(
            "{:10} {} [{}] - 距离: {:.2}m",
            item.element_type, item.id, item.name, distance
        );
    }
    println!();
    ...
    if !proximities.is_empty() {
        ...
        for (item, _, dist) in &proximities {
            println!("  ℹ️  {} [{}] - 间距: {:.3}m", item.name, item.id, dist);
        }
    }
    println!();
    ...
    println!(
        "  SCTN桥架数: {}",
        scene.iter().filter(|i| i.element_type == "SCTN").count()
    );
    println!(
        "  支架数量: {}",
        scene.iter().filter(|i| i.element_type == "SUPPO").count()
    );
