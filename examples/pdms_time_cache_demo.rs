...
use aios_database::fast_model::aabb_cache::{
    AabbCache, PdmsTimeExtractor, RefnoTimeData, SesnoTimeMapping,
};
...
async fn demo_database_1112(cache: &AabbCache) -> anyhow::Result<()> {
    ...
    for (refno, element_type, desc) in demo_refnos.iter().take(3) {
        if let Some(time_data) = cache.get_refno_time_data(*refno, 200) {
            let datetime =
                std::time::UNIX_EPOCH + std::time::Duration::from_secs(time_data.sesno_timestamp);
            println!(
                "  📌 RefNo: {}, 类型: {}, 描述: {}",
                refno.to_string(),
                element_type,
...
async fn demo_refno_history(cache: &AabbCache) -> anyhow::Result<()> {
    ...
    for (i, record) in history.iter().enumerate() {
        let datetime =
            std::time::UNIX_EPOCH + std::time::Duration::from_secs(record.sesno_timestamp);
        println!(
            "  {}. Session: {}, 时间: {:?}",
            i + 1,
            record.session,
...
async fn demo_time_range_query(cache: &AabbCache) -> anyhow::Result<()> {
    ...
    for refno in &refnos_in_range {
        ...
        let records_in_range: Vec<_> = history
            .iter()
            .filter(|record| {
                record.sesno_timestamp >= start_time && record.sesno_timestamp <= end_time
            })
            .collect();
